const createBoardGame = (rows, columns) => {
  return Array(rows)
    .fill(0)
    .map((_, row) => {
...
const createMinedBoardGame = (rows, columns, minesAmount) => {
  const board = createBoardGame(rows, columns)
...
const unexplored = field =>
  (field.mined && !field.flagged) || (!field.mined && !field.opened)

const winner = board => fields(board).filter(unexplored).length === 0

const exibhMines = board =>
  fields(board)
...
const markedFlags = board => fields(board).filter(field => field.flagged).length
